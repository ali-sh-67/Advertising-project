...
@section('content')
    <div style="width:1200px;height:777px;border:1px solid black;margin:5px auto;background-color:seashell;">
        <div style="width:200px;height:754px;border:1px solid black;float:right;margin-top:15px;margin-right:15px;background-color:#fff;line-height:40px ;font-size:18px; ">
            ...
            <div style="margin-right:54px;float:right;"><a style="text-decoration: none;color:blue;" href="{{route('allfavoriteAd')}}">همه علاقه مندیها</a></div> 
            
            <form method="post" action="{{route('showfavoriteAd')}}">
                 @csrf
                 <input type="hidden" name="favorite" value="not"/>
                 <a style="margin-right:53px;float:right;cursor: pointer;text-decoration: none;color:blue;" onclick="this.parentNode.submit();">مشاهده علاقه مندیها</a>
            </form>
            @if($online=='Admin')
             <div style="margin-right:54px;float:right;"><a style="text-decoration: none;color:blue;" href="{{route('category.create')}}">ایجاد دسته بندی</a></div> 
            @endif  
